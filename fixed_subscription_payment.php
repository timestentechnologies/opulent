...
try {
    ...
    $check_query = "SELECT * FROM user_subscriptions WHERE id = $subscription_id AND customer_id = $user_id";
    ...
    // Only add phone and code if they exist
    if (!empty($phone)) {
        $sql .= ", mobile = '$phone'";
    }
    ...
    $sql .= " WHERE id = $subscription_id AND customer_id = $user_id";
