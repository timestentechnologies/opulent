        ...
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full relative">
            <div class="p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 id="modalTitle" class="text-xl font-bold text-gray-900">Order Placed Successfully!</h3>
...
function showSuccessModal(data) {
    ...
    // Reset modal state - always start fresh
    ...
    if (data.isContact === true) {
        // Contact form success - handle this case first and separately
        modalTitle.textContent = 'Message Sent Successfully!';
        successMessage.classList.remove('hidden');
        successMessage.querySelector('p').textContent = data.message || 'Thank you for your message. We will get back to you soon!';
        iconContainer.classList.add('bg-green-100');
        icon.classList.add('ri-check-line', 'text-green-500');
        
        // Force hide order details for contact messages
        orderDetails.style.display = 'none';
        ...
        // Only show order details if we actually have data
        if (data.tracking_number || data.service_name) {
            orderDetails.style.display = 'block';
            
            // Update order details with safeguards against undefined values
            ...
            // If no order details, show a generic success message
            successMessage.classList.remove('hidden');
            successMessage.querySelector('p').textContent = data.message || 'Operation completed successfully!';
        }
    ...
    // Auto-close after 3 seconds and reload the page only for order success
    setTimeout(() => {
        closeSuccessModal();
        // Only reload for order successes, not contact forms or other successes
        if (data.success && !data.isContact && (data.tracking_number || data.service_name)) {
